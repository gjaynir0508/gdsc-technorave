...
export async function pushQuestions() {
    for(let i = 0; i < combinations.length; i++) {
        ...
        await sql`INSERT INTO s1 (id, clue1, clue2, clue3, passcode) VALUES (${q[0]}, ${q[1]}, ${q[2]}, ${q[3]}, ${passcodes[i]})`;
    }
}
...
export async function addNewPlayer(roll:number, name:string) {
    ...
    await sql`UPDATE s1 SET roll = ${roll}, name = ${name}, starttime = ${new Date().toISOString()} WHERE id = ${curLen}`;
    const q = qRes.rows[0];
    await sql`COMMIT`;
